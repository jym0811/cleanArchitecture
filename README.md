...
계층간 데이터를 주고 받을 때나 비즈니스 로직에서 하나의 객체를 타입이 
다른 객체로 형(Type) 변환하거나 여러 객체를 다른 객체로 합치는 일을 개발자가 모두 직접하게 되면
생산성이 떨어지므로 이런 종류의 라이브러리를 사용한다.

# ModelMapper

MapStruct와 다른 점은 런타임 시점에 Reflection API를 사용하여 객체를 매핑한다는 것이다
장점으로는 간결한 코드 작성과 필드 변경 사항에 대해 고려하지 않아도 된다는 점
단점으로는 바이트 코드 생성 방식을 이용하기에 디버깅이 어렵고 MapStruct 에 비해 처리속도 등의
성능이 떨어진다.


# MapStruct 
Annotation 을 이용해 메서드 인자와 반환할 값이 될 객체에 필요한 메서드를 호출하여 자동으로 객체 간의 매핑을 제공한다.  
장점으로는 컴파일 시점에 코드를 생성하기 떄문에 매핑이 불가능한 상태 등의 문제가 발생한 경우
컴파일 에러를 발생시키기 때문에 상대적으로 런타임에서 안정성을 보장하고. 처리속도가 ModelMaper 에 비해 훨씬 빠릅니다.
단점으로는 Lombok 충돌이 발생할 수 있다.


보통 MapStruct 사용이 권장된다.
...
@Valid
주로 request body 를 검증하는데 많이 사용된다.
자바의 표준 스펙이며 빈 검증기를 통해 객체의 제약 조건을 검증하도록 지시하는 어노테이션이다
...
@Validated는 Spring 프레임워크에서 제공하는 어노테이션 및 기능이다. @Validated를 사용하면 컨트롤러, 서비스, 레포지토리 등 계층에 
무관하게 스프링 빈이라면 유효성 검증을 진행할 수 있다. 대신 클래스에는 유효성 검증 AOP가 적용되도록 @Validated를 검증을 진행할 메소드에는 @Valid를 선언해주어야 한다.
...
웹 어댑터(인커밍 어댑터)는 HTTP 요청을 수신하여 파라미터와 콘텐츠를 객체로 역직렬화 해야한다.
보통은 웹 어댑터가 인증과 권한 부여를 수행하고 실패할 경우 에러를 반환한다. (유효성 검증)

유효성 검증은 유스케이스 입력 모델의 책임으로 설명하였으나 웹어댑터의 입력모델은 유스케이스 입력 모델과
다를 수 있으므로 또 다른 유효성 검증을 수행해야한다.

웹 어댑터의 입력 모델을 유스케이스 입력 모델로 변환할 수 있다는 것을 검증해야한다.


# 작명규칙

각 연산에 대해 가급적이면 별도의 패키지 안에 별도의 컨트롤러를 만드는 것을 추천한다.
또한 가급적 메서드와 클래스명은 유스케이스를 최대한 반영해서 지어야 한다.
계좌를 생성하는 방법은 계좌를 등록하는 방법뿐이므로 CreateAccount 보다 RegisterAccount 가 더 명확하다.
이렇게 되면 서로 다른 연산에 대해 코드를 짜고 있다면 병합 충돌이 일어날 확률이 줄어들 것이다.

--------------------------------------------------------------------------------------------------------------------
6장정리

영속성 어댑터는 아웃고잉 어댑터이다. 애플리케이션에 의해 호출될 뿐 호출하지는 않기 때문이다.
영속성 계층의 의존성을 없애기 위해 포트와 같은 간접 계층을 추가하고 있다.

# 영속성 어댑터 나누기

모든 영속성 포트를 구현하는 한 하나 이상의 클래스 생성을 금지하는 규칙은 없다.
영속성 연산이 필요한 도메인 클래스 하나당 하나의 영속성 어댑터를 구현할 수 있다.
이는 영속성 요구사항을 분리하기 좋은 토대가 된다.

풍부한 도메인 모델(Rich domain)을 생성하고 싶다면 도메인 모델과 영속성 모델을 매핑하는 것이 좋다.
